...
<PageMeta
	title="Documentazione e manuale di nostrfy"
	description="Il manuale completo di nostrfy: installazione, configurazione, supporto NIP, API REST, guide di distribuzione e risoluzione dei problemi per il motore relay Nostr tutto-in-uno."
/>

<div class="mx-auto max-w-3xl pb-20 pt-12">
	<h1 class="text-3xl font-bold tracking-tight sm:text-4xl">Documentazione</h1>
	<p class="mt-3 max-w-2xl text-base text-zinc-400">
		Tutto ciò che serve per installare, configurare, gestire e scalare un relay nostrfy — con il riferimento completo delle funzionalità e una guida alla risoluzione dei problemi mantenuta sul comportamento reale del relay.
	</p>
	...
	<div class="mt-12 rounded-xl border border-line bg-surface/40 p-5 text-sm text-zinc-400">
		Serve aiuto? Apri una issue su
		<a href={site.github} target="_blank" rel="noopener noreferrer" class="text-accent-300 hover:underline">
			GitHub</a
		>
		con i passaggi per riprodurre il problema e il log — la documentazione è mantenuta sul comportamento reale del relay e le correzioni sono benvenute.
	</div>
</div>
